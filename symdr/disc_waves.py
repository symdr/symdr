...
class DiscreteLatexPrinter(LatexPrinter):
    ...
    def _print_DiscreteGrid(self, expr):
        ...
        if expr._is_diff:
            return base_name + "_{" + diff + "}"
        elif expr._is_shifted or not self.ignore_grid:
            return base_name + "_{" + self.doprint(x_displ) + "}" \
                             + "^{" + self.doprint(t_displ) + "}"
        else:
            return base_name
    ...
    def _print_Add(self, expr):
        ...
        cont_str = f"\left[{super()._print_Add(cont_add)}\\right]_i^n"
        self.ignore_grid = False
    ...
    def _print_Mul(self, expr):
        ...
        self.ignore_grid = True
        cont_str = f"\left[{super()._print_Mul(cont_mul)}\\right]_i^n"
        self.ignore_grid = False
...
# TODO: type hints and throw errors at bad inputs
# TODO: separate continuous and discrete parts SOMEHOW
...
class DiscreteGrid(Expr):
    def __new__(cls, desig, x_shift=i, t_shift=n,
    ...
    def _is_shifted(self):
        return self.args[1] != i or self.args[2] != n
    ...
    def _eval_subs(self, lol, loler):
        if self._is_shifted:
            shifted_x = x + (self.args[1] - i) * h
            shifted_t = t + (self.args[2] - n) * tau
            return loler.subs({x: shifted_x, t: shifted_t})
        elif self._is_diff:
            return Derivative(loler, self.args[3], self.args[4])
        else:
            return loler
